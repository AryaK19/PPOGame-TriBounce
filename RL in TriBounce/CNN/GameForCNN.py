...
class Tribounce():
    ...
    def __init__(self,render=False) -> None:
        ...
        if self.render:
            ...
            self.screen = pygame.display.set_mode([self.width,self.height], flags=pygame.HIDDEN)
            pygame.display.set_caption("TriBounce")
        ...
        self.done = False
        self.state=np.zeros((700,1000,3))
    ...
    def run(self,action):
        ...
        if self.time > 2 and self.fpsCount%(1.7*self.fps)== 0 and len(self.enemyStats)<6:
            ran = rn.randint(0,1)
            if ran == 0:
        ...
        if self.playerPos[0]<200 or self.playerPos[0]>self.width-200:
            self.reward-=5
        ...
        self.state = np.rot90(self.state,axes=(0,1))    
    ...
    def reset(self):
        ...
        self.fpsCount = 0
        self.time = 0  
        self.state=np.zeros((700,1000,3),dtype=np.uint8)
